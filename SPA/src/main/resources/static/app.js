...
const REDIRECT_URI = 'http://localhost:5173/callback.html'; // SPA's callback
...
const callRockbandsButton = document.getElementById('callRockbandsButton');
const callSwedishpunsButton = document.getElementById('callSwedishpunsButton');
...
if (callRockbandsButton) {
    callRockbandsButton.addEventListener('click', () => callApi("rockbands"));
}
if (callSwedishpunsButton) {
    callSwedishpunsButton.addEventListener('click', () => callApi("swedishpuns"));
}
if (logoutButton) {
...
async function callApi(resource) {
    ...
    let endpoint = '/api/rockbands/random'; // fallback
    if (resource === 'swedishpuns') {
        endpoint = '/api/swedishpuns/random';
    }
    ...
    try {
        ...
        let output = ""
        if (endpoint === "/api/rockbands/random") {
            output += data.name
        } else {
            output += `${data.joke}\n${data.punchLine}`
        }
        apiResponseEl.textContent = output;
        statusEl.textContent = `API call to ${resource} successful.`;
...
function updateUI() {
    ...
    if (accessToken) {
        ...
        callRockbandsButton.style.display = 'inline-block';
        callSwedishpunsButton.style.display = 'inline-block';
